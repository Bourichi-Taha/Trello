  ...
  return (
    <AccordionItem value={organization.id} className="border-none" >
        ...
        <AccordionContent className="pt-1 text-neutral-700">
            {
                routes.map((route) => (
                    <Button variant={"ghost"} key={route.href} size={"sm"} onClick={()=>onClick(route.href)} className={cn("w-full font-normal justify-start pl-10 mb-1",pathname === route.href && "bg-sky-500/10 text-sky-700")}>
                        {
                            route.icon
                        }
                        {
